...
# documented 38 tools, all with distinct names.
func test_tools_list_exposes_all_38_distinct_tools() -> void:
	var h = McpHandler.new()
	var d := _parse(h.handle_message('{"jsonrpc":"2.0","id":40,"method":"tools/list","params":{}}'))
	var names := {}
	for t in d["result"]["tools"]:
		names[t["name"]] = true
	assert_eq(d["result"]["tools"].size(), 38, "tool count drifted from 38")
	assert_eq(names.size(), 38, "duplicate tool name registered")
