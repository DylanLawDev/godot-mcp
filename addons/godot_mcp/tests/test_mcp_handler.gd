...
func test_tools_list_exposes_all_distinct_tools() -> void:
	...
	for t in d["result"]["tools"]:
		names[t["name"]] = true
	var runtime_names := ["run_project", "get_run_status", "stop_project", "capture_game_frame", "send_input", "resize_game_window", "get_runtime_tree", "get_runtime_properties", "get_runtime_errors", "run_scenario", "get_scenario_result"]
